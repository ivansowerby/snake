...
class Ludus(Viewport):
    ...
    def __dictionary__(obj: Union[dict, object]) -> dict:
        if type(obj) == dict: return obj 
        elif isinstance(obj, object): return vars(obj)
        return {}
    ...
    def add_object(self, properties: Union[dict, object] = {}, gid: list = None) -> None:
        properties = Ludus.__dictionary__(properties)
        uid = self.new_object()
    ...
    def add_attribute(self, gid: str, attributes: Union[dict, object]) -> None:
        attributes = Ludus.__dictionary__(attributes)
